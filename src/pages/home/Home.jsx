                        ...
                        <div className="flex flex-col gap-2">
                            {cavaleiros.map((c, i) => (
                                <div key={c.nome} className="flex justify-between items-center">
                                    <label>{c.nome}</label>
                                    <input type="number" step="0.1" value={c.poderCosmico} onChange={(e) => updateCavaleiro(i, e.target.value)} className="bg-black border border-gray-500 text-white w-20 px-2 py-1 text-right" />
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {dificuldades.map((dif, i) => (
                            <div key={i} className="flex flex-col">
                                <label className="text-xs text-gray-400">Casa {i+1}</label>
                                <input type="number" value={dif} onChange={(e) => updateCasa(i, e.target.value)} className="bg-black border border-gray-500 text-white px-2 py-1" />
                            </div>
                        ))}
                    </div>
                </div>
